...
class Vector
{
  ...
  public:
	...
	Vector(const Vector &existingVector)
	{
		...
		{
			allocator[counter] = std::forward<T>(existingVector.allocator[counter]);
		};
	}
	...
	void erase(size_t num_of_pos)
	{
		...
		else if (this->sizeOfVector() == num_of_pos)
			this->pop_back();
	}
...
int main()
{
	...
	Vector<std::ofstream> anotherVec3(anotherVec1);

	anotherVec1 = std::move(anotherVec2);
	anotherVec1[0] << "must be ofs6.txt file";
	anotherVec3[0] << "\nHello! I am a copy constructor in ofs5.txt";
